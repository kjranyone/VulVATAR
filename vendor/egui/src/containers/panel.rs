...
impl SidePanel {
    ...
    fn show_inside_dyn<'c, R>(
        ...
        // ── VULVATAR VENDOR PATCH: side panels must not claim space
        // they don't paint ─────────────────────────────────────────
        // Upstream 0.30 (and through 0.34) clips panel painting to
        // `panel_rect` ("If we overflow, don't do so visibly", #4475)
        // but lets the frame's response rect — which is what
        // `PanelState`, the parent cursor and top-level
        // `allocate_left/right_panel` all take the panel's claimed
        // bounds from — follow the (unclipped) content. Content wider
        // than the panel therefore claims a strip the panel never
        // paints; the next panel starts past it and the window clear
        // colour bleeds through as a black band beside the panel.
        //
        // Clamp the content min_rect back to the panel allocation so
        // claim == paint. Clamp against `panel_rect` itself, NOT the
        // content ui's `max_rect`: widgets that don't fit expand both
        // min_rect AND max_rect, so by this point max_rect has already
        // been pushed out by the overflow and clamping to it is a
        // no-op. Overflowing content stays visually clipped, exactly as
        // #4475 intends.
        let patch_panel_rect = panel_rect;
        let patch_inner = frame.inner_margin;
        let inner_response = frame.show(&mut panel_ui, |ui| {
            ...
            let mut min_rect = ui.min_rect();
            min_rect.min.x = min_rect
                .min
                .x
                .max(patch_panel_rect.min.x + patch_inner.left);
            min_rect.max.x = min_rect
                .max
                .x
                .min(patch_panel_rect.max.x - patch_inner.right);
            ui.force_set_min_rect(min_rect);
