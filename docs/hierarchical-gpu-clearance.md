...
**クリアランス（Binding 6/7 — 親は内側表面）**: 外側頂点を親頂点の接平面に対して外へ押し出します。屈曲で2つの表面が折り重なる領域ではアンカー親法線が自頂点法線と逆向きになることがあり、そのまま押すと外側レイヤーを内側へ潰すため、**自頂点法線との内積が負のとき押し出し方向を反転**（褶を開く方向へ）させます。反転判定はワールド空間法線が必要なため、GPU 法線を持たない布頂点（rest 空間法線のまま）は反転なしの挙動を維持します。
...
if (ctrl.has_skin_anchors > 0u) {
    ...
    if (anc.body_vertex_idx != 0xFFFFFFFFu && anc.weight > 1e-4) {
        ...
        if (length(bp) > 1e-3 && length(bp) < 10.0) {
            vec3 eff_n = (dot(bn, world_nrm) < 0.0) ? -bn : bn;  // 褶での反転
