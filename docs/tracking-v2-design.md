...
| `VULVATAR_FUSION_NO_JUNKCLOUD` | ジャンククラウドゲート (§5) の無効化。既定はゲート ON |
| `VULVATAR_FUSION_ROLLORI` / `VULVATAR_ROLLORI_SIGMA` / `VULVATAR_ROLLORI_DUMP` | roll 専用 ori 観測 (§5) の有効化 (既定 OFF) / σ 上書き (既定 0.03+0.08·(1−conf)、手顔重なりで ×2) / 発火ログ |
| `VULVATAR_ABL_NOHEADKP` / `VULVATAR_ABL_NOEYES` / `VULVATAR_ABL_NOEARS` / `VULVATAR_ABL_NONOSE` / `VULVATAR_ABL_NOMESH` | 顔 kp 一括 / eye / ear / nose / canonical-face attach のみ除去 (ベンチ帰因用) |
