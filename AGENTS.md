...
- **hand モデルバックエンド (2026-09-17 改訂, 既定 MediaPipe)**: hand は `fusion/hands.rs` の `HandBackend` で MediaPipe と RTMPose-m hand crop モデルを切替 (`VULVATAR_HAND_BACKEND=rtmpose`。export 無しで付けると MediaPipe に warn フォールバック)。RTMPose モデル契約は `models/rtmpose-m-hand_256.onnx` (末尾 `_256` = square 入力サイズ、SimCC 出力 `(1,21,512)`×2、ImageNet 正規化 RGB)。**presence は SimCC ピーク sharpness の代理値で検出スコアではない**点に注意。handedness は 2D chirality 幾何推定 (掌/甲を区別できない既知限界、`VULVATAR_HAND_CHIRALITY_FLIP` で反転)。追加の拒否層: MP ラベルから蒸留した hand/背景 分類器 `models/rtmpose-hand-presence_64.onnx` (学習: `scratchpad/make_hand_presence_data.py` + `train_hand_presence.py`、**hands.csv の cx,cy は crop 左上座標**という罠あり) が models/ にあると読み込まれ、`VULVATAR_HAND_PRESENCE_VETO=<閾値>` (例 0.03) で「確信して手でない」候補を棄却できる (デスク背景 ~0.0 vs スライバ手 ~0.5)。provider 側には深度整合ゲート群 (`VULVATAR_HAND_NO_DEPTH_GATE` / `VULVATAR_HAND_DEPTH_TOL_M`) と temporal debounce も実装済み。実測: nohands 幻覚 317→4 ロック、clasp snaps 334→3/27 (duty MP 同等)、正面系は palms 救済 (9 vs 2 crops) 込みで MP 同等以上。**未達**: chin pose の acquisition (窓候補が手に乗らない) と namaste 被覆 (分類器が顔混同で veto しすぎ) — ラベルの顔混同問題は `scratchpad/hand_depth_first_path.md` と memory 参照。face は FaceMesh (`face_mediapipe.rs`) のみ — yolo26 hand/face 実験は A/B 不合格で放棄済み (コード削除済み)。追加の拒否層 (2026-09-17 深夜): (a) 幾何ゲート — 掌長/指関節幅比を [0.15,0.9] 帯に拘束 (SimCC の独立点デコードが顔/前腕で崩壊・過伸長するのを殺す、`VULVATAR_HAND_NO_GEOM_GATE=1` で無効)、(b) MP ラベル蒸留の hand/背景分類器 `models/rtmpose-hand-presence_64.onnx` (`scratchpad/{make_hand_presence_data,train_hand_presence}.py`、**hands.csv の cx,cy は crop 左上座標**という罠あり) による veto (`VULVATAR_HAND_PRESENCE_VETO=<閾値>`、デスク背景 ~0.0 vs スライバ手 ~0.5)。**SimCC 座標は bins 正規化後に crop size を掛ける** (model_size 割算のままだと 21 点全部が crop 原点 ±1px に潰れる — rt7〜rt14 の A/B はこのバグ込みの数値)。修正後の実測 (clasp): crops 747/779・duty 0.96/1.00・snaps 0/0 = MP 完全同等、nohands も幻覚なしで MP 同等、solve 28.7ms = MP 同速。**未達**: chin/wave/palms の acquisition (候補窓が手に乗らず coverage が MP 比 1 桁下、veto 閾値 0.0 でも chin 39/441)。**A/B 完全合格まで rtmpose は既定 MediaPipe のまま**。
