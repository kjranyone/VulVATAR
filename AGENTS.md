...
- **顔戦役 (MediaPipe face 削除 — 最後の MP 消費者, 進行中)**: `face_mediapipe.rs` (FaceMeshV2 478 + BlendshapeV2 52 → expressions) が残る唯一の MediaPipe 依存。削除までの段取り: (1) **蒸留ラベル生成** — `scratchpad/make_face_distill_data.py` が録画全フレームへ FaceMeshV2+BlendshapeV2 (runtime 同一 ONNX・同一前処理: nearest-pixel 256 NCHW [0,1]・146 点 subset ÷256) を 2 パス (候補 crop 探索 → `derive_face_bbox` 同関数で精密 crop) で実行し、box + 478 landmarks (crop/frame 両空間) + blendshape 52 + conf を `datasets/face_distill/` (manifest.jsonl + shard npz) へ出力。2026-09-18 実行済。(2) FaceMesh478→WFLW98 対応構築 (WFLW アノテーション `datasets/wflw_dl/` と mean template `datasets/wflw_yolo/face98_mean.json` を使用、anchor PASS/FAIL 検証付き)。(3) face98 検出器学習 (WFLW + ドメイン box)。(4) blendshape 蒸留 MLP 98→52。(5) head pose 定数校正 → (6) A/B。**潜在バグ発見 (2026-09-18)**: runtime の face_conf は `Identity_1` (常に −20 以下の死んだ logit) を sigmoid して読んでおり**常時 ~0** — 真の conf は `Identity_2` (既に sigmoid 済み [0,1])。ライブ顔追跡が body 由来信頼度で動いているため顕在化していないが、face 削除時の置換実装では `Identity_2` を使うこと。
