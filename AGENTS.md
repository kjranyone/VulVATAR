...
- **hand モデルバックエンド (2026-09-17 RTMPose two-stage 実装, 既定 MediaPipe)**: hand は `fusion/hands.rs` の `HandBackend` で MediaPipe と RTMPose を切替 (`VULVATAR_HAND_BACKEND=rtmpose`。export 無しで付けると MediaPipe に warn フォールバック)。モデル契約は 3 本 (`models/` は ignore 済み、export 物は commit されない): (1) `models/rtmpose-m-hand_256.onnx` — SimCC `(1,21,512)`×2、ImageNet 正規化 RGB。**SimCC 座標は bins 正規化後に crop size を掛ける** (model_size 割算のままだと 21 点が crop 原点に潰れる — rt7〜rt14 の A/B はこのバグ込みの数値)。(2) `models/rtmpose-hand-presence_64.onnx` — MP ラベル蒸留の hand/背景 64px 分類器 (**hands.csv の cx,cy は crop 左上座標**の罠あり)。学習は v3 (`scratchpad/make_hand_presence_v3_data.py` → datasets/hand_presence_v3): ランタイムモダリティ一致のため palm 峰值中心窓 (MP 裏付け付き) を positive に加え、正面 3 セッションは `datasets/frontal_labels/labels.json` (グリッドシート目視ラベル) から。desk holdout + nohands val 98.3% — ただし **net 初期化で運用挙動が振れる** (chin 60↔448、再学習したら必ず desk A/B を回すこと)。(3) `models/rtmpose-hand-palm_256.onnx` — 全フレーム letterbox 256 → 16×16 手首ヒートマップの提案ネット (MP 手首ラベルで蒸留、hard-negative mining 2 ラウンド; `scratchpad/{make_hand_palm_data,train_hand_palm}.py`)。デスク val recall 27-30/30、ピーク 0.99。
  **runtime パイプライン (2026-09-17 確定)**: presence は **presence net が唯一の権威** — provider が全候補を再スコアする (palm 候補 = ヒートマップ峰值中心、他 = デコード手首中心。旧 `VULVATAR_HAND_PRESENCE_VETO` env は廃止、net があるなら常時 ON)。SimCC sharpness は crop size 依存で誤窓デコードとは**逆相関** (cuff で 0.83) のため排他。幾何ゲート (掌長/指関節幅比 [0.15,0.9]、`VULVATAR_HAND_NO_GEOM_GATE=1` で無効) は estimate 内に残る。palm 候補は候補チェーン末尾に最大 2 つ (top-1 峰值 × 0.30/0.50×frame幅、手首オフセット (0.71, 0.74) は MP crop 統計)、深度サンプルも峰值で取る (デコード手首は chin pose で ~40px ずれ、窓が前腕布を覗いて net 0.00 vs 峰值中心 0.89)。深度ゲートは palm 専用ティア: 計測アンカー 0.25m / (net≥0.70 && FK 0.45m) / (net≥0.50 && FK 0.25m) / 前腕レイ (`palm_gate_tol_m`=0.45, `VULVATAR_HAND_NO_PALM=1` で palm 全無効, `VULVATAR_HAND_PALM_DEBUG=1` で tally)。chirality は 2D 幾何推定 (掌/甲不能の既知限界) だが net≥0.85 の palm 候補は hard veto を回避 (chin 拳は MP 学習頭と逆極を読む — 935 棄却の実測)、continuation は net≥0.50 で再 crop を継続 (sharpness は publish 帯を彷徨って hold/re-acq を往復 = snap の主因、23→9)、soft hold 帯は 0.20-0.50×3 フレーム。
  **A/B 実績 (rt29, ab_mp_* 比 / models は gitignore 済みのため再現は再学習が必要 — net 初期化で chin 60↔448 が振れる点に注意)**: chin L 448 (MP 441)・duty 0.92 (MP 0.88) = **被覆・duty とも MP同等以上**、ただし snaps 23 vs 4。正面系が解けた: wave L 10 (MP 4) duty 0.55 (0.31)、palms L 15 (MP 2) duty 0.79 (0.26)、namaste R duty 0.72 = MP 同等 — 2026-09-17 深夜のグリッドシート目視ラベル (`datasets/frontal_labels/`, 52 フレーム・装箱数+手首点) で両ネットを正面付き再学習し、presence を**ランタイムモダリティ一致** (palm 峰值中心窓を positive に) にした効果。clasp L 780・duty 1.0・snaps 0 だが **R 5 (MP 780) = 既知最大の未達**: modality 整合で両スロットの net が同一 blob に高スコアを付け、palm-src near-L 抑止と FK 3D ゲートで R の獲得が阻害される (rt27 は dupes 68、rt29 は事前抑止で dupes 0 と R 枯渇に転換)。nohands 73/12 (MP 20/31、snaps 27) もやや過剰。**次の一手 = スロット排他性**: detector wrist アンカー (実測 3D 点) を L/R どちらの候補かの裁定に使い、38px dupe 半径をアンカー間距離から動的に決める。solve 原価: 候補全 Fail フレームで 50-70ms (palm net + 候補チェーン) — ライブ 30Hz で要観測。**A/B 完全合格まで rtmpose は既定 MediaPipe のまま**。face は FaceMesh (`face_mediapipe.rs`) のみ — yolo26 hand/face 実験は放棄済み (コード削除済み)。
