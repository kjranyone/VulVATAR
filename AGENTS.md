...
- **hand モデルバックエンド (2026-09-17 RTMPose two-stage 実装, 既定 MediaPipe)**: hand は `fusion/hands.rs` の `HandBackend` で MediaPipe と RTMPose を切替 (`VULVATAR_HAND_BACKEND=rtmpose`。export 無しで付けると MediaPipe に warn フォールバック)。モデル契約は 3 本 (`models/` は ignore 済み、export 物は commit されない): (1) `models/rtmpose-m-hand_256.onnx` — SimCC `(1,21,512)`×2、ImageNet 正規化 RGB。**SimCC 座標は bins 正規化後に crop size を掛ける** (model_size 割算のままだと 21 点が crop 原点に潰れる — rt7〜rt14 の A/B はこのバグ込みの数値)。(2) `models/rtmpose-hand-presence_64.onnx` — MP ラベル蒸留の hand/背景 64px 分類器 (**hands.csv の cx,cy は crop 左上座標**の罠あり)。val 97.6%。(3) `models/rtmpose-hand-palm_256.onnx` — 全フレーム letterbox 256 → 16×16 手首ヒートマップの提案ネット (MP 手首ラベルで蒸留、hard-negative mining 2 ラウンド; `scratchpad/{make_hand_palm_data,train_hand_palm}.py`)。デスク val recall 27-30/30、ピーク 0.99。
  ...
  **A/B 実績 (rt21, ab_mp_* 比)**: clasp 780/780・snaps 0/0・duty 1.0/1.0・dupes 0 = **MP 完全同等**。chin 386/441 ロック・duty 0.83 vs 0.88・snaps 9 vs 4。nohands 15/11 ロック (MP 20/31 以下)・duty 同等・dupes 1。**未達 = 正面系 (wave/namaste-L 0 ロック vs MP 4-6)**: presence net のドメインギャップ — palm ヒートマップは正面の手を毎フレーム見つける (peak 0.86-0.95) が、正面ピーク中心窓は net 0.00。自己教師 (峰值を positive に) は**肩/天井 FP を positive として教え込み崩壊する** (`scratchpad/make_hand_presence_v2_data.py` に経緯) — 正面の手の 2D ラベルが無い以上、depth-first パス (`scratchpad/hand_depth_first_path.md` 項目 1: 手首深度観測昇格) が恒久解。solve 原価: 候補全Failフレームで 50-70ms (palm net + 候補チェーン) — ライブ 30Hz で要観測。**A/B 完全合格まで rtmpose は既定 MediaPipe のまま**。face は FaceMesh (`face_mediapipe.rs`) のみ — yolo26 hand/face 実験は放棄済み (コード削除済み)。
