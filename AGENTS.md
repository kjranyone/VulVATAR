...
- **hand モデルバックエンド (2026-09-17 改訂, 既定 MediaPipe)**: hand は `fusion/hands.rs` の `HandBackend` で MediaPipe と RTMPose-m hand crop モデルを切替 (`VULVATAR_HAND_BACKEND=rtmpose`。export 無しで付けると MediaPipe に warn フォールバック)。RTMPose モデル契約は `models/rtmpose-m-hand_256.onnx` (末尾 `_256` = square 入力サイズ、SimCC 出力 `(1,21,512)`×2、ImageNet 正規化 RGB)。**presence は SimCC ピーク sharpness の代理値で検出スコアではない**点に注意。handedness は 2D chirality 幾何推定 (掌/甲を区別できない既知限界、`VULVATAR_HAND_CHIRALITY_FLIP` で反転)。face は FaceMesh (`face_mediapipe.rs`) のみ — 2026-09-17 の yolo26 hand/face 実験 (ultralytics hand-keypoints / WFLW 学習) は A/B 不合格で**放棄済み**: 正面系受理 crop 0・dupes 666・solve 2倍 (yolo26 hand)、コードも剥がした (`face_yolo26.rs` 削除、`training/` 削除)。失敗の切り分け詳細は `scratchpad/hand_depth_first_path.md` (次の一手 = depth-first hand パス: crop 提案の 3D 中心化 → 3D L/R 割当 → 手首深度観測昇格)。**A/B 合格まで rtmpose は既定 MediaPipe のまま**。
