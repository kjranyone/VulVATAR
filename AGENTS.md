...
- **顔戦役 (MediaPipe face 削除 — 最後の MP 消費者, 進行中)**: `face_mediapipe.rs` (FaceMeshV2 478 + BlendshapeV2 52 → expressions) が残る唯一の MediaPipe 依存。削除までの段取り: (1) **蒸留ラベル生成** — `scratchpad/make_face_distill_data.py` が録画全フレームへ FaceMeshV2+BlendshapeV2 (runtime 同一 ONNX・同一前処理: nearest-pixel 256 NCHW [0,1]・146 点 subset ÷256) を 2 パス (候補 crop 探索 → `derive_face_bbox` 同関数で精密 crop) で実行し、box + 478 landmarks (crop/frame 両空間) + blendshape 52 + conf を `datasets/face_distill/` (manifest.jsonl + shard npz) へ出力。2026-09-18 実行済。(2) **FaceMesh478→WFLW98 対応 ✅ 完了 (2026-09-18)** — `scratchpad/build_face98_mapping.py`: WFLW test アノテーション 149 枚 → similarity-ICP 整列 → 距離カーネル重み付き投票 (σ=1.5% span) → 全局貪欲割当 (一意制約; 画像毎の argmin は隣接 MP 点と入れ替わるためハード投票は使用不可) → **98/98 PASS・衝突ゼロ**。検証: in-sample dist_rel median 0.26%、holdout 25 面 median 3.4% (annotation-style bias 込み・輪郭点集中 = 誤対応なら 10-30%)、iris anchor WFLW 96→MP 470 / 97→475 正解。出力 `datasets/face_distill/mp478_to_wflw98.json`。FaceMesh conf ヘッドは WFLW 写真ドメインで ~0 を読む (landmarks は正常追跡) — ゲートは必ず幾何で行う。(3) face98 検出器学習 (WFLW + ドメイン box)。(4) blendshape 蒸留 MLP 98→52。(5) head pose 定数校正 → (6) A/B。**潜在バグ発見 (2026-09-18)**: runtime の face_conf は `Identity_1` (常に −20 以下の死んだ logit) を sigmoid して読んでおり**常時 ~0** — 真の conf は `Identity_2` (既に sigmoid 済み [0,1])。ライブ顔追跡が body 由来信頼度で動いているため顕在化していないが、face 削除時の置換実装では `Identity_2` を使うこと。
