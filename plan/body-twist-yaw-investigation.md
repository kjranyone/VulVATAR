...
- **Option A は 2026-05-01 に部分撤回**。当初は肩ペアと腰ペアの信頼度
  重み付き平均で合成するよう変更したが、徹底コードレビューで 45°/315°
  の三クォータ姿勢において `45°` 行の腰 Δz が肩 Δz と **符号が逆** に
  出ていることが判明 (肩 Δz=+0.004 に対し腰 Δz=-0.014)。これは「肩と腰が
  同じヨーの独立サンプル」という Option A の前提が崩れていることを示し、
  平均化すると 45°/315° で **アバターのヨー符号が反転** する回帰を生む。
  腰ペアの三クォータ姿勢での Δz は spine twist ではなく RTMW3D の depth
  ノイズに支配されている可能性が高い。
- 暫定対応として `compute_body_yaw_3d` を「肩ペアを最優先、肩が見えない/
  低信頼の場合のみ腰ペアにフォールバック」のシンプルな順次選択に書き換え
  (2026-05-01)。Option A の **ロバスト性ゲインのうち、信頼度ゲートで肩を
  使わない部分は維持** ・ **平均化部分は破棄** という整理。これで sign-
  flip 回帰は消えたが、当初の症状である「45° で胴体が回らない」は元の
  ~1° に戻っている。
- 新しい `body_yaw_tests` (8 ケース) は計測表の (a) 45° 三クォータが
  小さくも正符号、(b) 90°/270° で +48°/+62°、(c) 180° で ±π 近傍、
  (d) 肩腰が符号で衝突しても肩優先、(e) 肩のみ/腰のみフォールバック、
  (f) 全空入力で `None` をカバー。
- 残課題: 45° 三クォータで実用的なヨーを得るには Option B (4点平面
  フィット)、または Option C (肘 Δz を上腕姿勢ゲート付きで併用) に
  移行する必要がある。Option A の「平均」アプローチは取らない。
- 実機検証 (`gothic_lolita_rotation/photorealistic/` 8 枚) は別途。

## 影響ファイル

- `src/avatar/pose_solver.rs` `compute_body_yaw_3d` — 肩ペア最優先 +
  腰ペアフォールバックの順次選択。dead-zone 閾値 (0.05) は据え置き。
- `src/avatar/pose_solver.rs` `body_yaw_tests` モジュール — 計測表に
  基づく符号正当性アサーションへ刷新。
