...
- [x] **Folder watcher のエッジケースが未検証** — 修正済み。
  (1) `src/app/folder_watcher.rs` モジュール docstring に Limitations
  セクションを追記: UNC/SMB は Windows で server-side イベント未配信、
  OneDrive/Google Drive/Dropbox は placeholder swap で `Modified` に
  集約され `Created` を失う可能性、symlink/junction は platform 依存。
  (2) 既存の `poll_folder_watcher` から library 追加ロジックを
  `import_vrm_into_library_if_missing` 関数に抽出 (notify 経路と
  manual 経路の outcome 同一化)。
  (3) `GuiApp::refresh_watched_folders` を新設: 各 watched dir を walk
  して未登録 .vrm を import + 結果を notification で報告。
  (4) Inspector の Watched folders セクションに "Refresh" ボタン
  (hover hint 付き) と「Local filesystem 推奨、cloud-sync は best-effort」
  注釈を追加。
  (5) 3 unit test:
  - 既登録パスは re-add しない
  - Refresh で missed VRM が library に入る + 2 回目は no-op
  - watched dir 0 件 Refresh は "no new files" toast のみ。
  既存 `#[ignore]` 付き test は notify backend の non-determinism 由来
  なのでそのまま (CI 安定性優先)、代わりに上記 manual 経路で挙動を pin。
