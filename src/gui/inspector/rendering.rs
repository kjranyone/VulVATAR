...
pub(super) fn draw_scene(ui: &mut egui::Ui, state: &mut GuiApp) {
    ...
    collapsible_card(ui, "inspector.material_mode", t!("inspector.material_mode"), false, |ui| {
            ui
    ...
    collapsible_card(ui, "inspector.lighting", t!("inspector.lighting"), true, |ui| {
    ...
    collapsible_card(ui, "inspector.composition", t!("inspector.composition"), false, |ui| {
            ui
                .add(
    ...
    collapsible_card(ui, "inspector.physics", t!("inspector.physics"), false, |ui| {
            ...
            if rapier_active {
                ...
                ui.label(egui::RichText::new(t!("inspector.rapier_not_init")).color(color::ON_SURFACE_MUTED));
            }
...
fn draw_avatar_transform(ui: &mut egui::Ui, state: &mut GuiApp) {
    collapsible_card(ui, "inspector.transform", t!("inspector.avatar_transform"), true, |ui| {
...
fn draw_scene_background(ui: &mut egui::Ui, state: &mut GuiApp) {
    collapsible_card(ui, "inspector.scene_background", t!("inspector.scene_background"), false, |ui| {
...
fn draw_global_gravity(ui: &mut egui::Ui, state: &mut GuiApp) {
    collapsible_card(ui, "inspector.gravity_global", t!("inspector.gravity_global"), false, |ui| {
...
fn draw_scene_presets(ui: &mut egui::Ui, state: &mut GuiApp) {
    collapsible_card(ui, "inspector.scene_preset.presets", t!("inspector.scene_preset.presets"), false, |ui| {
            ui.horizontal(|ui| {
                ...
                .clicked()
                {
                    if let Some(idx) = state.scene_preset.selected_index {
                        if idx < state.scene_preset.presets.len() {
                            ...
                            if let Err(e) = crate::persistence::save_scene_presets(
                                &state.scene_preset.presets,
                            ) {
                                log::warn!("persistence: save_scene_presets failed: {e}");
