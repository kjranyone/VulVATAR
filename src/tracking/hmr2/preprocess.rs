//! 256×256 NCHW resize + ImageNet normalize for HMR2.
//!
//! HMR2 trims a 32-px band off each side internally
//! (`x[:, :, :, 32:-32]`) to land on its 192×256 ROI, so the caller
//! provides a full square 256×256 and the model handles the aspect
//! shift. ImageNet normalisation in `[0, 255]` to match ViTPose's
//! convention — same `(value - mean) / std` arithmetic, just at
//! HMR2's resolution.
...
pub fn preprocess(rgb: &[u8], src_w: u32, src_h: u32) -> Array4<f32> {
    let dst = IMAGE_SIZE as usize;
