//! YOLO11-pose detector — the production single-stage whole-frame person
...
//!   filled from YOLO11 and every other block (foot / face-68 / both
...
//!   cascade as RTMW3D, fed a bbox derived from YOLO11's five face
//!   points (nose, eyes, ears) instead of the face-68 block.
//! * The σ the estimator consumes is synthesised (YOLO11 regresses
...
//! `diagnose_yolo11_pose`; the acceptance gate is the fusion replay
...
/// Synthesised localisation σ for YOLO11 keypoints, in frame pixels
...
/// Calibration factor applied to YOLO11's raw per-joint confidence before
...
/// Extra inflation on the face bbox derived from YOLO11's five face
/// points. RTMW3D derives its crop from the face-68 spread (chin to
/// forehead ≈ face height); YOLO11's points span roughly the ear-to-ear
...
pub(crate) struct Yolo11PoseInference {
    session: Session,
    input_name: String,
    output_name: String,
...
impl Yolo11PoseInference {
    pub(crate) fn from_models_dir_with_options(
        models_dir: impl AsRef<Path>,
        opts: DetectorOptions,
    ) -> Result<Self, String> {
        let model_path = models_dir.as_ref().join("yolo11-pose.onnx");
        let model_path = if model_path.exists() {
            model_path
        } else {
            // Fall back through the export-filename convention used by
            // scratchpad/export_yolo11_pose.py (prefer the fastest).
            let candidates = ["yolo11n-pose_480.onnx", "yolo11n-pose_640.onnx"];
            let mut found = None;
            for c in candidates {
            ...
            found.ok_or_else(|| {
                "YOLO11-pose model not found in models dir (expected yolo11-pose.onnx \
                 or yolo11n-pose_*.onnx)"
                    .to_string()
            })?
        };

        let (session, backend) = if opts.force_cpu {
            super::session::build_session_cpu_only(&model_path.to_string_lossy(), 4, "YOLO11-pose")?
        } else {
            build_session(&model_path.to_string_lossy(), 4, "YOLO11-pose")?
        };
        ...
        info!(
            "YOLO11-pose loaded {} (input '{}x{}', output '{}'), backend {}",
            model_path.display(),
            input_name,
            size,
            output_name,
    ...
    pub(crate) fn estimate_pose(
        ...
        if let Some(ref kps) = kps17 {
            ...
            // YOLO11's per-joint confidence is a raw head sigmoid — badly
            ...
            for (i, slot) in joints.iter_mut().take(17).enumerate() {
                ...
                *slot = DecodedJoint {
                    ...
                    // near 0. YOLO11 has no z head; leaving zscore at 0
        ...
        debug!(
            "YOLO11-pose timing: total={:>5.1}ms det={:>5.1}ms person={}",
    ...
    fn detect(
        ...
        let input = match TensorRef::from_array_view(&tensor) {
            Ok(v) => v,
            Err(e) => {
                warn!("YOLO11-pose: tensor view failed: {e}");
            ...
            .run(ort::inputs![self.input_name.as_str() => input])
        {
            Ok(o) => o,
            Err(e) => {
                warn!("YOLO11-pose: run failed: {e}");
                return (None, None, t0.elapsed().as_secs_f32() * 1000.0);
            }
        };
        let Some(out) = outputs.get(self.output_name.as_str()) else {
            warn!("YOLO11-pose: output '{}' missing", self.output_name);
            return (None, None, t0.elapsed().as_secs_f32() * 1000.0);
        };
        let Ok((_, data)) = out.try_extract_tensor::<f32>() else {
            warn!("YOLO11-pose: output not f32");
            return (None, None, t0.elapsed().as_secs_f32() * 1000.0);
        };
        if data.len() % 56 != 0 {
            warn!(
                "YOLO11-pose: unexpected output size {} (expected 56×A)",
                data.len()
            );
            return (None, None, t0.elapsed().as_secs_f32() * 1000.0);
        }
...
/// Face bbox from YOLO11's body-frame face points (nose / eyes / ears),
